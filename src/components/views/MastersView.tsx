...
export const MastersView: React.FC<MastersViewProps> = ({
  ...
  const [activeMaster, setActiveMaster] = useState<
    'projects' | 'customers' | 'vendors' | 'banks' | 'cash_accounts' | 'business_partners' | 'expense_heads'
  >('projects');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
            Projects, Customers, Vendors, Bank Accounts, Cash &amp; Petty Cash Accounts, Business Partners, and Expense Heads
          </p>
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-2 shadow-xs flex flex-wrap gap-1.5">
        ...
        <button
          onClick={() => setActiveMaster('business_partners')}
          className={`px-3 py-2 text-xs font-semibold rounded-lg transition-colors cursor-pointer ${
            activeMaster === 'business_partners'
              ? 'bg-slate-900 dark:bg-blue-600 text-white'
              : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800'
          }`}
        >
          Business Partners ({state.businessPartners.length})
        </button>
        <button
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xs overflow-hidden">
        ...
        {activeMaster === 'business_partners' && (
          <div>
            <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
              <div>
                <h3 className="text-sm font-semibold text-slate-800 dark:text-white">Business Partners Master</h3>
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
              ...
              <div className="flex items-center gap-2">
                {canImportMasterData && (
                  <button
                    onClick={() => setImportModalType('business_partners')}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/60 hover:bg-emerald-100 dark:hover:bg-emerald-900 border border-emerald-200 dark:border-emerald-800 cursor-pointer transition-colors"
                  >
                    <UploadCloud className="w-3.5 h-3.5" />
                    <span>Import Business Partners</span>
                  </button>
                )}
                <button
                  onClick={onOpenNewBusinessPartner}
                  className="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium rounded-lg text-white bg-teal-700 hover:bg-teal-600 cursor-pointer"
                >
                  <Plus className="w-3.5 h-3.5" />
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {state.businessPartners.map((p) => (
                    <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                      ...
                      <td className="py-2.5 px-4 text-slate-700 dark:text-slate-300">{p.contactPerson || '—'}</td>
                      <td className="py-2.5 px-4 text-right font-mono text-slate-700 dark:text-slate-300">{formatOMR(p.openingBalance)}</td>
                      <td className="py-2.5 px-4 text-right font-mono font-bold text-teal-600 dark:text-teal-400">{formatOMR(p.currentBalance)}</td>
                      <td className="py-2.5 px-4">
                        <span className="px-2 py-0.5 rounded text-[10px] font-semibold bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800">
                          {p.status}
                        </span>
