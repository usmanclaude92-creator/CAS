...
import { downloadTransferImportTemplate } from '../../utils/transferImportTemplate';
import {
  parseTransferImportRows,
  classifyTransferImportRows,
  TransferImportRowResult,
} from '../../utils/transferImportValidation';
...
export const TransferImportModal: React.FC<TransferImportModalProps> = ({ isOpen, onClose, onSuccess }) => {
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [phase, setPhase] = useState<ImportPhase>('upload');
  const [fileName, setFileName] = useState('');
  const [rowResults, setRowResults] = useState<TransferImportRowResult[]>([]);
  ...
  const authCheck = authService.verifyTransferImportAuthority();
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-rose-300 dark:border-rose-900 shadow-2xl max-w-md w-full p-6 text-center space-y-4">
          ...
          <p className="text-xs text-slate-600 dark:text-slate-300">
            {authCheck.error ||
              'Transfer import requires the "transfers.import" permission, grantable to a role under Roles & Permissions.'}
          </p>
          <button
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const raw = parseTransferImportRows(grid);
      if (raw.length === 0) {
      ...
      const results = classifyTransferImportRows(raw, state);
  ...
  const handleConfirmImport = async () => {
    ...
    for (const row of newRows) {
      ...
      try {
        await accountingService.createTransfer({
          date: r.date,
          transferFromType: r.transferFromType,
          transferFromId: r.transferFromId,
          transferToType: r.transferToType,
    ...
    accountingService.addAuditLog(
      'BULK_IMPORT_TRANSFERS',
      'Banking & Treasury',
      `${currentUser.fullName} imported Bank/Cash Transfers from "${fileName}": ${postedCount} new, ${failedRows.length} failed. Total rows in file: ${rowResults.length}.`
    );
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-2xl max-w-3xl w-full max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-6 pt-6 pb-3 border-b border-slate-100 dark:border-slate-800 shrink-0">
          <div className="flex items-center gap-2 text-slate-900 dark:text-white font-bold text-sm">
            ...
            <span>Import Bank/Cash Transfers</span>
          </div>
          <button
            type="button"
            onClick={handleCloseModal}
            ...
            <>
              <div className="p-3 bg-amber-50 dark:bg-amber-950/40 rounded-lg border border-amber-200 dark:border-amber-800/60 text-xs text-amber-900 dark:text-amber-200 flex items-start gap-2.5">
                ...
                <div>
                  ...
                  <p className="mt-0.5 text-[11px] text-amber-800 dark:text-amber-300">
                    Only users whose role holds the "transfers.import" permission may run this import. Transfers
                    cannot be edited once posted — a row exactly matching an existing transfer (same date, amount,
                    accounts, and document ref) blocks the whole file until fixed.
                  </p>
                </div>
              </div>
              ...
              >
                ...
                <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">
                  Transfer From/To Account must match existing Bank, Cash, Petty Cash, or Business Partner records exactly.
                </p>
              </div>
              ...
              <button
                type="button"
                onClick={downloadTransferImportTemplate}
            ...
            <>
              <div className="grid grid-cols-2 gap-2 text-center text-[11px]">
                <div className="bg-slate-50 dark:bg-slate-800/60 p-2 rounded-lg">
                  <span className="block text-slate-500 dark:text-slate-400 uppercase">New Transfers</span>
              ...
              {newRows.length > 0 && (
                <div>
                  <div className="text-[11px] font-semibold text-slate-600 dark:text-slate-300 mb-1.5">
                    New transfers to post ({newRows.length})
                  </div>
                    ...
                    {newRows.map((row) => (
                      <div key={row.rowNumber} className="p-2.5 text-[11px] flex items-center justify-between gap-2">
                        <span className="text-slate-700 dark:text-slate-300">
                          Row {row.rowNumber} — {row.resolved?.date} · {row.resolved?.transferFromName} →{' '}
                          {row.resolved?.transferToName}
                        </span>
          ...
          {phase === 'result' && summary && (
            ...
            >
              <div className="flex items-center gap-2 font-semibold">
                ...
                <span>
                  {summary.success
                    ? `Import complete: ${summary.postedCount} transfer(s) posted.`
