...
import { downloadMoneyInImportTemplate } from '../../utils/moneyInImportTemplate';
import {
  parseMoneyInImportRows,
  classifyMoneyInImportRows,
  MoneyInImportRowResult,
} from '../../utils/moneyInImportValidation';
import { formatOMR } from '../../utils/formatters';

interface MoneyInImportModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: () => void;
}
...
export const MoneyInImportModal: React.FC<MoneyInImportModalProps> = ({ isOpen, onClose, onSuccess }) => {
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [phase, setPhase] = useState<ImportPhase>('upload');
  const [fileName, setFileName] = useState('');
  const [rowResults, setRowResults] = useState<MoneyInImportRowResult[]>([]);
  ...
  const authCheck = authService.verifyMoneyInImportAuthority();
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-rose-300 dark:border-rose-900 shadow-2xl max-w-md w-full p-6 text-center space-y-4">
          ...
          <p className="text-xs text-slate-600 dark:text-slate-300">
            {authCheck.error ||
              'Historical Money In import requires the "money_in.import" permission, grantable to a role under Roles & Permissions.'}
          </p>
          <button
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const raw = parseMoneyInImportRows(grid);
      if (raw.length === 0) {
      ...
      const results = classifyMoneyInImportRows(raw, state);
  ...
  const handleConfirmImport = async () => {
    ...
    for (const row of newRows) {
      ...
      try {
        await accountingService.recordMoneyIn({
          ...
          remarks: r.remarks ? `${r.remarks} (historical import)` : 'Imported via historical Money In bulk import',
        });
        postedCount++;
      } catch (err: any) {
    ...
    for (const row of updateRows) {
      try {
        await accountingService.fillMissingMoneyInFields(row.matchedExistingId!, {
          customerId: row.fillableFields?.customerId,
    ...
    accountingService.addAuditLog(
      ...
      'Banking & Treasury',
      `${currentUser.fullName} imported historical Money In transactions from "${fileName}": ${postedCount} new, ${updatedCount} updated, ${failedRows.length} failed. Total rows in file: ${rowResults.length}.`
    );
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-2xl max-w-3xl w-full max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-6 pt-6 pb-3 border-b border-slate-100 dark:border-slate-800 shrink-0">
          <div className="flex items-center gap-2 text-slate-900 dark:text-white font-bold text-sm">
            ...
            <span>Import Historical Money In Transactions</span>
          </div>
          <button
            type="button"
            onClick={handleCloseModal}
            ...
            <>
              <div className="p-3 bg-amber-50 dark:bg-amber-950/40 rounded-lg border border-amber-200 dark:border-amber-800/60 text-xs text-amber-900 dark:text-amber-200 flex items-start gap-2.5">
                ...
                <div>
                  ...
                  <p className="mt-0.5 text-[11px] text-amber-800 dark:text-amber-300">
                    Only users whose role holds the "money_in.import" permission may run this import. Every row
              ...
              >
                ...
                <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">
                  Project, Customer Name, and Account Name must match existing records exactly.
                </p>
              </div>
              ...
              <button
                type="button"
                onClick={downloadMoneyInImportTemplate}
                    ...
                    {updateRows.map((row) => (
                      <div key={row.rowNumber} className="p-2.5 text-[11px] flex items-center justify-between gap-2">
                        ...
                        <span className="text-blue-700 dark:text-blue-300 font-medium">
                          {[
                            row.fillableFields?.customerName ? `Customer → ${row.fillableFields.customerName}` : null,
                    ...
                    {newRows.map((row) => (
                      <div key={row.rowNumber} className="p-2.5 text-[11px] flex items-center justify-between gap-2">
                        <span className="text-slate-700 dark:text-slate-300">
                          Row {row.rowNumber} — {row.resolved?.transactionDate} · {row.resolved?.projectName}
                        </span>
...
export default MoneyInImportModal;
