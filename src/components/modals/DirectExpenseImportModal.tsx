...
import { downloadDirectExpenseImportTemplate } from '../../utils/directExpenseImportTemplate';
import {
  parseDirectExpenseImportRows,
  classifyDirectExpenseImportRows,
  DirectExpenseImportRowResult,
} from '../../utils/directExpenseImportValidation';
import { formatOMR } from '../../utils/formatters';

interface DirectExpenseImportModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: () => void;
}
...
export const DirectExpenseImportModal: React.FC<DirectExpenseImportModalProps> = ({ isOpen, onClose, onSuccess }) => {
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [phase, setPhase] = useState<ImportPhase>('upload');
  const [fileName, setFileName] = useState('');
  const [rowResults, setRowResults] = useState<DirectExpenseImportRowResult[]>([]);
  ...
  const authCheck = authService.verifyDirectExpenseImportAuthority();
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const raw = parseDirectExpenseImportRows(grid);
      if (raw.length === 0) {
      ...
      const results = classifyDirectExpenseImportRows(raw, state);
  ...
  const handleConfirmImport = async () => {
    ...
    for (const row of updateRows) {
      try {
        await accountingService.fillMissingDirectExpenseFields(row.matchedExistingId!, {
          vendorName: row.fillableFields?.vendorName,
            ...
            <>
              <div className="p-3 bg-amber-50 dark:bg-amber-950/40 rounded-lg border border-amber-200 dark:border-amber-800/60 text-xs text-amber-900 dark:text-amber-200 flex items-start gap-2.5">
                ...
                <div>
                  ...
                  <p className="mt-0.5 text-[11px] text-amber-800 dark:text-amber-300">
                    Only users whose role holds the "expenses.import" permission may run this import. Every row
              ...
              >
                ...
                <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">
                  Project, Expense Head, and Account Name must match existing records exactly. Vendor Name is
                  free text.
                </p>
              </div>
              ...
              <button
                type="button"
                onClick={downloadDirectExpenseImportTemplate}
            ...
            <>
              <div className="grid grid-cols-3 gap-2 text-center text-[11px]">
                <div className="bg-slate-50 dark:bg-slate-800/60 p-2 rounded-lg">
                  <span className="block text-slate-500 dark:text-slate-400 uppercase">New Expenses</span>
                    ...
                    {updateRows.map((row) => (
                      <div key={row.rowNumber} className="p-2.5 text-[11px] flex items-center justify-between gap-2">
                        <span className="text-blue-900 dark:text-blue-100">
                          Row {row.rowNumber} — {row.resolved?.expenseDate}, {formatOMR(row.resolved?.grossAmount || 0)}
                        </span>
              ...
              {newRows.length > 0 && (
                <div>
                  <div className="text-[11px] font-semibold text-slate-600 dark:text-slate-300 mb-1.5">
                    New expenses to post ({newRows.length})
                  </div>
                    ...
                    {newRows.map((row) => (
                      <div key={row.rowNumber} className="p-2.5 text-[11px] flex items-center justify-between gap-2">
                        <span className="text-slate-700 dark:text-slate-300">
                          Row {row.rowNumber} — {row.resolved?.expenseDate} · {row.resolved?.expenseHeadName}
                        </span>
          ...
          {phase === 'result' && summary && (
            ...
            >
              <div className="flex items-center gap-2 font-semibold">
                ...
                <span>
                  {summary.success
                    ? `Import complete: ${summary.postedCount} new expense(s) posted, ${summary.updatedCount} existing record(s) updated.`
...
export default DirectExpenseImportModal;
