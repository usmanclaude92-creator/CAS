...
import { downloadMoneyOutImportTemplate } from '../../utils/moneyOutImportTemplate';
import {
  parseMoneyOutImportRows,
  classifyMoneyOutImportRows,
  MoneyOutImportRowResult,
} from '../../utils/moneyOutImportValidation';
import { formatOMR } from '../../utils/formatters';

interface MoneyOutImportModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: () => void;
}
...
export const MoneyOutImportModal: React.FC<MoneyOutImportModalProps> = ({ isOpen, onClose, onSuccess }) => {
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [phase, setPhase] = useState<ImportPhase>('upload');
  const [fileName, setFileName] = useState('');
  const [rowResults, setRowResults] = useState<MoneyOutImportRowResult[]>([]);
  ...
  const authCheck = authService.verifyMoneyOutImportAuthority();
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-rose-300 dark:border-rose-900 shadow-2xl max-w-md w-full p-6 text-center space-y-4">
          ...
          <p className="text-xs text-slate-600 dark:text-slate-300">
            {authCheck.error ||
              'Payment to Vendors import requires the "money_out.import" permission, grantable to a role under Roles & Permissions.'}
          </p>
          <button
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const raw = parseMoneyOutImportRows(grid);
      if (raw.length === 0) {
      ...
      const results = classifyMoneyOutImportRows(raw, state);
  ...
  const updateRows = rowResults.filter((r) => r.status === 'update');
  ...
  const handleConfirmImport = async () => {
    ...
    for (const row of newRows) {
      ...
      try {
        await accountingService.recordMoneyOut({
          ...
          remarks: r.remarks ? `${r.remarks} (bulk import)` : 'Imported via Payment to Vendors bulk import',
        });
        postedCount++;
      } catch (err: any) {
    ...
    for (const row of updateRows) {
      try {
        await accountingService.fillMissingMoneyOutFields(row.matchedExistingId!, {
          vendorId: row.fillableFields?.vendorId,
          remarks: row.fillableFields?.remarks,
        });
        updatedCount++;
      } catch (err: any) {
        failedRows.push({ rowNumber: row.rowNumber, message: err?.message || 'Unknown error updating this row.' });
      }
    }
    ...
    accountingService.addAuditLog(
      ...
      'Purchases & Payables',
      `${currentUser.fullName} imported Payment to Vendors transactions from "${fileName}": ${postedCount} new, ${updatedCount} updated, ${failedRows.length} failed. Total rows in file: ${rowResults.length}.`
    );
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-2xl max-w-3xl w-full max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-6 pt-6 pb-3 border-b border-slate-100 dark:border-slate-800 shrink-0">
          <div className="flex items-center gap-2 text-slate-900 dark:text-white font-bold text-sm">
            ...
            <span>Import Payment to Vendors Transactions</span>
          </div>
          <button
            type="button"
            onClick={handleCloseModal}
            ...
            <>
              <div className="p-3 bg-amber-50 dark:bg-amber-950/40 rounded-lg border border-amber-200 dark:border-amber-800/60 text-xs text-amber-900 dark:text-amber-200 flex items-start gap-2.5">
                ...
                <div>
                  ...
                  <p className="mt-0.5 text-[11px] text-amber-800 dark:text-amber-300">
                    Only users whose role holds the "money_out.import" permission may run this import. Every row
                    posts or updates exactly like a manual entry — same account balance effect, same journal
                    entry, same audit log — and the whole file is validated before anything is written.
                  </p>
                </div>
              </div>
              ...
              >
                ...
                <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">
                  Project, Vendor Name, Expense Head, and Account Name must match existing records exactly.
                </p>
              </div>
              ...
              <button
                type="button"
                onClick={downloadMoneyOutImportTemplate}
            ...
            <>
              <div className="grid grid-cols-3 gap-2 text-center text-[11px]">
                <div className="bg-slate-50 dark:bg-slate-800/60 p-2 rounded-lg">
                  <span className="block text-slate-500 dark:text-slate-400 uppercase">New Transactions</span>
                  <strong className="text-emerald-700 dark:text-emerald-300 text-sm">{newRows.length}</strong>
                </div>
                <div className="bg-slate-50 dark:bg-slate-800/60 p-2 rounded-lg">
                  <span className="block text-slate-500 dark:text-slate-400 uppercase">Existing Records to Update</span>
                  <strong className="text-blue-700 dark:text-blue-300 text-sm">{updateRows.length}</strong>
                </div>
                <div className="bg-slate-50 dark:bg-slate-800/60 p-2 rounded-lg">
              ...
              {updateRows.length > 0 && (
                <div>
                  <div className="flex items-start gap-2 text-[11px] text-blue-800 dark:text-blue-200 mb-1.5">
                    <Info className="w-4 h-4 shrink-0 mt-0.5 text-blue-600 dark:text-blue-400" />
                    <span>These rows match an already-posted transaction; only the listed blank field(s) will be filled in — nothing that already has a value is changed.</span>
                  </div>
                  <div className="rounded-lg border border-blue-200 dark:border-blue-800 divide-y divide-blue-100 dark:divide-blue-900 max-h-40 overflow-y-auto">
                    {updateRows.map((row) => (
                      <div key={row.rowNumber} className="p-2.5 text-[11px] flex items-center justify-between gap-2">
                        <span className="text-blue-900 dark:text-blue-100">
                          Row {row.rowNumber} — {row.resolved?.transactionDate}, {formatOMR(row.resolved?.amount || 0)}
                        </span>
                        <span className="text-blue-700 dark:text-blue-300 font-medium">
                          {[
                            row.fillableFields?.vendorName ? `Vendor → ${row.fillableFields.vendorName}` : null,
                            row.fillableFields?.remarks ? `Remarks → "${row.fillableFields.remarks}"` : null,
                          ]
                            .filter(Boolean)
                            .join(', ')}
                        </span>
              ...
              {newRows.length > 0 && (
                <div>
                  <div className="text-[11px] font-semibold text-slate-600 dark:text-slate-300 mb-1.5">
                    New transactions to post ({newRows.length})
                  </div>
                  <div className="rounded-lg border border-slate-200 dark:border-slate-800 divide-y divide-slate-100 dark:divide-slate-800 max-h-40 overflow-y-auto">
                    {newRows.map((row) => (
                      <div key={row.rowNumber} className="p-2.5 text-[11px] flex items-center justify-between gap-2">
                        <span className="text-slate-700 dark:text-slate-300">
                          Row {row.rowNumber} — {row.resolved?.transactionDate} · {row.resolved?.paidTo}
                        </span>
                        <span className="font-mono font-semibold text-slate-900 dark:text-white">
                          {formatOMR(row.resolved?.amount || 0)}
                        </span>
                      </div>
                    ))}
                  </div>
          ...
          {phase === 'result' && summary && (
            ...
            >
              <div className="flex items-center gap-2 font-semibold">
                ...
                <span>
                  {summary.success
                    ? `Import complete: ${summary.postedCount} new transaction(s) posted, ${summary.updatedCount} existing record(s) updated.`
                    : `Import partially completed before a row failed: ${summary.postedCount} posted, ${summary.updatedCount} updated, ${summary.failedRows.length} failed.`}
                </span>
              </div>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-slate-100 dark:border-slate-800 shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            {phase === 'review' && (
              ...
              >
                {isProcessing ? 'Importing…' : `Confirm & Import (${newRows.length + updateRows.length} row${newRows.length + updateRows.length === 1 ? '' : 's'})`}
              </button>
            )}
          </div>
        </div>
...
export default MoneyOutImportModal;
